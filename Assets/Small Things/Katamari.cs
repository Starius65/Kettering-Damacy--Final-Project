...
namespace Klonamari
{
    public class Katamari : MonoBehaviour
    {
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            volume = 4.0f / 3.0f * Mathf.PI * Mathf.Pow(sphere.radius*scale, 3); //initial volume calculated by radius of the sphere.
        ...
        {
            if (collision.gameObject.layer == 10 | collision.gameObject.layer == 5)
            {
                return;
            }
            ...
            if (!rolledUp)
            {
                ...
                float sphereBottom = transform.position.y - sphere.radius*scale;
                if (collision.gameObject.layer == 8 && targetTop > sphereBottom && sphereBottom + STAIR_CLIMB_RATIO * (sphere.radius * scale) > targetTop)
                {
                    ++defaultContacts;
            ...
            if (collectible)
            {
                if (collectible.mass < mass * ROLL_UP_MAX_RATIO)
                {
                    if (!collectible.collected)
                    {
                        ...
                        float distance = delta.magnitude - ((sphere.radius * scale));
        ...
        {
            sphere.radius = Mathf.Pow((3 * volume) / (4 * Mathf.PI), ONE_THIRD) / scale;
        ...
        public void ControlSwitch(bool b)
        {
            EnableControls = b;
            if (!b)
        ...
        void UpdateSize()
        {
            float inCM = sphere.radius * 100f;
            int cM = int.Parse(inCM.ToString().Substring(0, (int)Mathf.Log10(inCM)));
            int mM = Mathf.RoundToInt((inCM - cM * 10));
            //Debug.Log(inCM +" "+ cM +" "+ mM);
            sizeFormated = string.Format("{0:0}cm {1:0}mm",
                cM,
                mM);
            SizeBox.text = sizeFormated;
        }

        public string SizeFormatted() { return sizeFormated; }
        public float SizeFloat() { return sphere.radius; }
        public void BecomeStar(bool b) { star = b; }
    }
}